...
> the row is removed with it, and an expired one is cleared by the next
> reconcile. Keep in mind the recorded address is the one **you invited** — not
...
rows by email — and `gh teacher roster import` reads the same file: it corrects a
pending row's name and section, matched by address, without touching the
invitation. Only the id-keyed rows differ, since `import` resolves students by
username; it skips them with a notice and leaves whatever is stored for them
