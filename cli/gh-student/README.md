...
**The autograde shim template lives in the shared module**, at `cli/shared/contract/autograde-shim.yaml`, rendered by `contract.RenderDefaultShim`. Both `gh student accept` and `gh teacher assignment enable-autograder` (which adds the shim to repos accepted while the built-in autograder was off) render the same bytes from it, so the two binaries cannot drift. `accept.go` embeds nothing of its own and consumes the seams above like every other command; its placement at the module root is a convention, no longer an embed constraint (the embed-terminus learning this README used to link describes the earlier layout).
