...
// element across pages. Mirrors the teacher CLI's helper of the same name so
// neither binary hand-rolls the walk.
//
...
func PaginateAll[T any](
	...
	for page := 1; page <= maxPages; page++ {
		...
		// Centralized termination via ghutil.NextPage (shared with the teacher
...
// Link header. It uses RequestWithContext (not Get) so the Link header is
// available for next-page resolution.
//
