...
The runner-side bootstrap (`.github/scripts/runner.py`), the score collector (`.github/scripts/collect_scores.py`), the declarative-test materializer (`.github/scripts/materialize_tests.py`), and the Feedback-PR maintainer (`.github/scripts/ensure_feedback_pr.py`) live under `skeleton/dotgithub/scripts/` and ship under `.github/scripts/` in each org's `classroom50` repo (nested under `.github/` so the directory name can't collide with a classroom slug). The runner is fetched from Pages by the autograde-runner reusable workflow at workflow runtime on every submission (the student-repo shim only `uses:` the runner workflow — it never performs Pages fetches itself). The runner resolves the entrypoint: per-assignment `<classroom>/autograders/<slug>/autograder.py` if present in the bundle, otherwise the classroom default at `<classroom>/autograder.py`, otherwise a vacuous-pass synthesis. The autograder workflow shim template lives in the shared module (`cli/shared/contract/autograde-shim.yaml`, rendered by `contract.RenderDefaultShim`); `gh student accept` drops it into each student repo at accept time, and `gh teacher assignment enable-autograder` adds it to repos accepted while the built-in autograder was off.
