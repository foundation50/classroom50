...
func assignmentSubmissionModeCmd() *cobra.Command {
	...
	cmd.Flags().BoolVar(&updateShims, "update-shims", true, "Retrofit existing student repos' shims (--update-shims=false changes only assignments.json)")
	cmd.Flags().StringVar(&user, "user", "", "Retrofit only this student's <classroom>-<slug>-<user> repo")
