...
const AssignmentsTable = ({
  ...
  //
  // `submitted`: graded `entries` plus the bucket's `detected` list, the
  // commits/tags collect_scores.py observed in repos with no graded entry (every
  // submitter of a no_autograder assignment; for an autograded one, pushes the
  // autograder hasn't published yet). Counting both is what keeps this cell in
  // step with the Submissions page, which shows those repos as "Pending". The
  // union is by owner: a teacher can hand-grade a no_autograder assignment,
  // which writes entries for owners detection also lists (#659).
  //
  // `notCollected`: a `detected` key that is absent (not `[]`) means no
  // collect has walked the bucket yet, which is NOT "nobody submitted". Only
  // claimed for no_autograder, whose entries are never autogenerated; an
  // autograded bucket written before detection existed has entries but no key.
  //
  ...
  const funnelCounts = (assignment: Assignment) => {
    const gradedRows = scoresData?.submissions?.[assignment.slug] ?? []
    const detectedRows = scoresData?.detected?.[assignment.slug]
    const gradedOwners = new Set(
      gradedRows.flatMap((row) =>
        typeof row.owner === "string" ? [row.owner.toLowerCase()] : [],
      ),
    )
    const detectedOnly = (detectedRows ?? []).filter(
      (row) => !gradedOwners.has(row.owner.toLowerCase()),
    ).length
    const submitted = gradedRows.length + detectedOnly
    const notCollected =
