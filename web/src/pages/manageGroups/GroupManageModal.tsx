...
// read-only summaries. Writes are delegated to the page, which owns the
// mutations, the snapshot resync, and the shared error string surfaced here.
// The page remounts it per open (a session key) so draft state starts fresh,
// and keeps `team` set through the close fade so content never blanks.
        ...
        >
          {({ id, describedById }) => (
            <div className="flex items-center gap-2">
              ...
              <Button
                variant="outline"
                disabled={busy || nameDraft.trim() === (team.name ?? "")}
