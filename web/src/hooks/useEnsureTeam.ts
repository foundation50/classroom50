...
// owner-only. Gate the query on resolved org ownership so a TA/instructor never
// fires a guaranteed-403 team creation, and use the fail-closed retry predicate
// so a definitive 403/404 doesn't retry (a plain useQuery otherwise retries the
// forbidden write with backoff). `unresolved` holds until ownership is known.
const useEnsureTeam = (org: string, classroom: string) => {
  ...
  const teamQuery = useQuery({
    ...
    enabled: Boolean(org) && Boolean(classroom) && orgRole === "owner",
