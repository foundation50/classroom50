...
// student).
// The teacher signal probes the `-teacher` team.
...
export function useClassroomRole(
  ...
  const actualRole = resolveClassroomRole({
    org,
    classroom,
    teacher: membershipFromQuery(teacherQuery.isSuccess, teacherQuery.error),
  ...
  const isError =
    ...
    !isLoading &&
    (teacherQuery.isError || htaQuery.isError || taQuery.isError)
  ...
  const refetch = useCallback(() => {
    ...
    void refetchTa()
    void refetchStudent()
  }, [refetchTeacher, refetchHta, refetchTa, refetchStudent])
