import { useTranslation } from "react-i18next"
import { WorkflowIcon } from "@/components/ui/icons"

import { Alert, Modal, ModalIcon } from "@/components/ui"
import {
  BulkPhaseFooter,
  BulkProgressBlock,
  BulkResultBody,
} from "@/components/bulk/resultView"
import {
  outcomeSection,
  ownerDisplayName,
  runBulkFanOut,
  type FanOutOutcome,
} from "@/components/bulk/fanOut"
import { useBulkRun } from "@/components/bulk/useBulkRun"
import {
  addAutogradeShim,
  type ShimBackfillOutcome,
} from "@/domain/assignments/shimBackfill"
import { useGitHubClient } from "@/context/github/GitHubProvider"
import { REPO_WRITE_CONCURRENCY } from "@/github-core/queries"
import { getRepo } from "@/github-core/repoReads"
import { CONFIG_REPO, DEFAULT_BRANCH } from "@/util/configRepo"
import { studentRepoName } from "@/util/studentRepo"
import type { Student, SubmissionMode } from "@/types/classroom"

type BulkAutogradeShimModalProps = {
  open: boolean
  onClose: () => void
  org: string
  classroom: string
  assignment: string
  // The assignment's STORED submission_mode and submission_tags: the shim is
  // rendered with the same trigger accept would produce today.
  submissionMode: SubmissionMode
  submissionTags?: string[]
  // Accepted students; each login is the owner segment of their own repo.
  owners: string[]
  students?: Student[]
}

type Outcome =
  | FanOutOutcome
  | { owner: string; status: ShimBackfillOutcome["status"]; detail?: string }

// Whole-assignment shim backfill: add the built-in autograding workflow to
// every accepted student repo that lacks one, in one bounded fan-out. The way
// to reconcile repos accepted while the assignment had the built-in autograder
// off, once the teacher turns it on. Sibling of BulkSubmissionTriggerModal.
export function BulkAutogradeShimModal({
  open,
  onClose,
  org,
  classroom,
  assignment,
  submissionMode,
  submissionTags,
  owners,
  students = [],
}: BulkAutogradeShimModalProps) {
  const { t } = useTranslation()
  const client = useGitHubClient()
  const bulk = useBulkRun(open)
  const { phase, progress, result, busy } = bulk

  const total = owners.length
  const displayFor = ownerDisplayName(students)

  const run = async () => {
    if (total === 0) return
    if (!bulk.begin(total)) return
    // The reusable-workflow ref every shim points at; one read for the run.
    // Fail closed: a guessed branch would be baked into files that are never
    // rewritten, so a failed read stops the run before any write.
    let configBranch: string
    try {
      const config = await getRepo(client, org, CONFIG_REPO)
      configBranch = config?.default_branch || DEFAULT_BRANCH
    } catch {
      if (!bulk.isMounted()) return
      bulk.complete(
        {
          headline: t("submissions.bulkShim.configBranchError"),
          sections: [],
        },
        "error",
      )
      return
    }
    if (!bulk.isMounted()) return
    // A confirmed missing workflow scope stops the rest: every remaining repo
    // would fail identically.
    let missingScope = false

    const { outcomes } = await runBulkFanOut<Outcome>({
      owners,
      // Each iteration is a 3-step git-data WRITE into a different repo;
      // GitHub's secondary-rate-limit guidance is to avoid concurrent content
      // writes (the CLI loop is serial for the same reason).
      concurrency: REPO_WRITE_CONCURRENCY,
      shouldStop: () => missingScope,
      isMounted: bulk.isMounted,
      onProgress: (processed, owner) =>
        bulk.setProgress({ processed, total, message: displayFor(owner) }),
      t,
      perOwner: async (owner) => {
        const repo = studentRepoName(classroom, assignment, owner)
        const outcome = await addAutogradeShim({
          client,
          org,
          repo,
          configBranch,
          submissionMode,
          submissionTags,
        })
        if (outcome.status === "missingWorkflowScope") missingScope = true
        return {
          owner,
          status: outcome.status,
          detail:
            outcome.status === "unrecognized" ? outcome.reason : undefined,
        }
      },
    })
    if (!bulk.isMounted()) return

    const added = outcomes.filter((o) => o.status === "added")
    const present = outcomes.filter((o) => o.status === "present")
    const unrecognized = outcomes.filter((o) => o.status === "unrecognized")
    const notAccepted = outcomes.filter((o) => o.status === "notAccepted")
    const deferred = outcomes.filter((o) => o.status === "deferred")
    const scope = outcomes.filter((o) => o.status === "missingWorkflowScope")
    const failed = outcomes.filter((o) => o.status === "failed")

    const section = (titleKey: string, rows: Outcome[], detail?: string) =>
      outcomeSection(t, titleKey, rows, displayFor, detail)

    bulk.complete(
      {
        headline: t("submissions.bulkShim.resultHeadline", {
          added: added.length,
          present: present.length,
          total,
        }),
        sections: [
          ...section(
            "submissions.bulkShim.scopeSection",
            scope,
            t("submissions.bulkShim.scopeDetail"),
          ),
          ...section("submissions.bulkShim.failedSection", failed),
          ...section("submissions.bulkShim.unrecognizedSection", unrecognized),
          ...section(
            "submissions.bulkShim.deferredSection",
            deferred,
            t("submissions.bulkShim.deferredDetail"),
          ),
          ...section(
            "submissions.bulkShim.notAcceptedSection",
            notAccepted,
            t("submissions.bulkShim.notAcceptedDetail"),
          ),
        ],
      },
      failed.length || scope.length || deferred.length ? "error" : "complete",
    )
  }

  return (
    <Modal
      open={open}
      onClose={onClose}
      closeDisabled={busy}
      size="lg"
      title={t("submissions.bulkShim.title")}
      subtitle={t("submissions.bulkShim.subtitle", { count: total })}
      headerVisual={
        <ModalIcon>
          <WorkflowIcon className="size-4" aria-hidden="true" />
        </ModalIcon>
      }
      footer={
        <BulkPhaseFooter
          phase={phase}
          busy={busy}
          showApply={total > 0}
          applyLabel={t("submissions.bulkShim.apply")}
          onApply={() => void run()}
          onClose={onClose}
        />
      }
    >
      {phase === "idle" && (
        <div className="mt-4 flex flex-col gap-4">
          {total === 0 ? (
            <Alert tone="info" className="text-sm">
              {t("submissions.bulkShim.noRepos")}
            </Alert>
          ) : (
            <Alert tone="warning" className="text-sm">
              <div className="flex flex-col gap-1">
                <span>
                  {t("submissions.bulkShim.warningLead", { count: total })}
                </span>
                <ul className="ms-4 list-disc space-y-0.5">
                  <li>{t("submissions.bulkShim.warningPoint1")}</li>
                  <li>{t("submissions.bulkShim.warningPoint2")}</li>
                  <li>{t("submissions.bulkShim.warningPoint3")}</li>
                </ul>
              </div>
            </Alert>
          )}
        </div>
      )}

      {busy && (
        <BulkProgressBlock
          workingLabel={t("submissions.bulkShim.working")}
          progress={progress}
          caption={t("submissions.bulkShim.progress", {
            processed: progress.processed,
            total: progress.total,
          })}
        />
      )}

      <BulkResultBody phase={phase} result={result}>
        <Alert tone="info" className="text-sm">
          <div className="flex flex-col gap-1">
            <span>{t("submissions.bulkShim.repullReminder")}</span>
            <span>{t("submissions.bulkShim.regradeReminder")}</span>
          </div>
        </Alert>
      </BulkResultBody>
    </Modal>
  )
}

export default BulkAutogradeShimModal
